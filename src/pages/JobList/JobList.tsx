...
import { jobList } from '../../util/api';

type Props = {};

const JobList = (props: Props) => {
  const [data, setData] = useState<JobListModel[]>([]);
  const param = useParams();
  const keyword: string | undefined = param.keyword;
  const id: string | undefined = param.id;

  const getListById = async () => {
    const dataApi = await jobList.getListById(id);
    setData(dataApi);
  };

  const getListByKeyword = async () => {
    const dataApi = await jobList.jobSearch(keyword);
    setData(dataApi);
  };
  useEffect(() => {
    if (param.keyword) {
          ...
          <div className='joblist__result'>
            ...
            <div className='result__sort'>
              <div className='result__info'>
                <span>{data.length} services available</span>
              </div>
              <div className='sort-by'>
                ...
                <div className='sort-menu'>
                  ...
                  <div className='menu'>
                    <div className='menu-item'>Relevance</div>
                    <div className='menu-item'>Best Selling</div>
                    <div className='menu-item'>Newest Arrivals</div>
                  </div>
                </div>
              </div>
            </div>
            <div className='result__services'>
              <div className='service__content row'>
                {data.map((list: JobListModel, index: number) => {
                  return (
                    <div
                      key={index}
