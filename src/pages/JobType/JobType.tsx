...
import { jobType } from '../../util/api';

const JobType = () => {
  const [data, setData] = useState<JobTypeModel[]>([]);
  const params = useParams();

  const getTypeById = async () => {
    const dataApi = await jobType.getTypeById(params.id);
    setData(dataApi);
  };
        ...
        <div className='jobtype__content'>
          ...
          <div className='popular__job'>
            <div className='popular__job-title'>
              <h4>Most popular in {data[0]?.tenLoaiCongViec}</h4>
            </div>
          ...
          {data.map((type: JobTypeModel, index: number) => {
            return (
